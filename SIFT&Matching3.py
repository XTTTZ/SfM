...
tripoints3d=np.zeros(shape=(4,1))
img=cv.imread('E:\\2_Univercity\\3_3_Abroad\\2_RoboticUIUC\\3_Paper\\git\\3Dreconstruction\\imgs\\dinos\\ (1).ppm',1)
K = np.array(camMat)
...
for x in range(3):
    img1=cv.imread('E:\\2_Univercity\\3_3_Abroad\\2_RoboticUIUC\\3_Paper\\git\\3Dreconstruction\\imgs\\dinos\\ ('+str(x+1)+').ppm',1)#-1，0，1表示加载方式，0是灰度，1是彩色，-1是alpha通道，默认彩色
    img2=cv.imread('E:\\2_Univercity\\3_3_Abroad\\2_RoboticUIUC\\3_Paper\\git\\3Dreconstruction\\imgs\\dinos\\ ('+str(x+4)+').ppm',1)
    ...
    for i, (m, n) in enumerate(matches):
        ...
        if m.distance < 0.8 * n.distance:#阈值,不同照片这里可能并不是最佳阈值
            good.append(m)
    ...
    u=np.array(u)
    u1=np.array(u1)
    tripoints3d=np.hstack(    (tripoints3d,structure.linear_triangulation(u.T, u1.T, P, P1)))


#tripoints3d = np.delete(tripoints3d, 1)
fig = plt.figure()
